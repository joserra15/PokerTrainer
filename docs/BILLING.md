...
supabase secrets set STRIPE_PROMO_COUPON_ID=wcTGqarh
supabase secrets set STRIPE_PROMO_CODE_ID=promo_...
```

Promoción **SUMMER26** (Live): cupón `wcTGqarh`, 50 % dto. una vez. El usuario lo introduce en Checkout (`allow_promotion_codes`). Debe existir un **Promotion Code** con texto `SUMMER26` vinculado al cupón (no basta con el cupón solo).
