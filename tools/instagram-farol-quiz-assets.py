#!/usr/bin/env python3
"""Genera puzzles + soluciones Instagram '¿Con qué faroleas mejor?' estilo PokerForgeAI.

Cartas: mazo de cuatro colores (igual que data-card-style=colored en la app).
Las 3 opciones son aires candidatos a farol; la correcta tiene el mejor blocker.

Criterios: marketing/instagram/10-farol-quiz/CRITERIOS.md
"""
...
OUT = ROOT / "marketing/instagram/10-farol-quiz"
DATA = OUT / "casos.json"
...
def draw_header(draw, solution: bool):
    ...
    title = "Solución: ¿con qué faroleabas mejor?" if solution else "¿Con qué faroleas mejor?"
...
def draw_board_and_hero(img, case, y0):
    """Board + Tú (reversos: eliges farol) + Villano; stacks visibles."""
    ...
    n_board = len(case["board"])
    board_w = n_board * cw + (n_board - 1) * gap
    hero_w = 2 * cw + gap
    sep = 36
    ...
    draw.text((hero_x, y0), f"Tú {case['heroPos']}", font=f_lab, fill=MUTED)
    hw = draw.textbbox((0, 0), f"Tú {case['heroPos']} ", font=f_lab)[2]
    ...
    # Reversos: aún no revelamos con qué faroleas
    draw_card(img, hero_x, cy, "Xx", cw, ch, hidden=True)
    draw_card(img, hero_x + cw + gap, cy, "Xx", cw, ch, hidden=True)
...
def draw_options(img, case, y0, reveal_idx=None):
    ...
    f_h = font(24, True)
    f_lab = font(18, True)
    draw.text((40, y0), "¿Con cuál faroleas? (elige una)", font=f_h, fill=WHITE)
    y = y0 + 36
...
def validate_case(case: dict):
    ...
    if len(board) != len(set(board)):
        raise ValueError(f"Case {case['n']}: duplicate board {board}")
    for i, opt in enumerate(case["options"]):
        combo = board + opt["cards"]
...
def main():
    ...
    lines = [
        "# Farol Quiz — 30 casos Instagram",
        "",
        "Puzzles y soluciones numerados en archivo (01–30). **Sin número en la imagen.**",
        "Pregunta: ¿Con qué faroleas mejor? Cartas: mazo de cuatro colores.",
        "",
    ...
    lines += [
        ...
        "",
        "Regenerar: `python3 tools/instagram-farol-quiz-assets.py`",
        "",
    ]
