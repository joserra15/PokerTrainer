-- FOUNDER Study / Coach: flags y solicitudes por plan.

alter table public.pt_user_profiles
  add column if not exists is_founder_study boolean not null default false;

alter table public.pt_user_profiles
  add column if not exists is_founder_coach boolean not null default false;

alter table public.pt_user_profiles
  add column if not exists founder_study_requested_at timestamptz;

alter table public.pt_user_profiles
  add column if not exists founder_coach_requested_at timestamptz;

-- Migrar flag legado is_founder → Study
update public.pt_user_profiles
set is_founder_study = true
where coalesce(is_founder, false) = true
  and coalesce(is_founder_study, false) = false;

update public.pt_user_profiles
set founder_study_requested_at = coalesce(founder_study_requested_at, founder_requested_at)
where founder_requested_at is not null
  and founder_study_requested_at is null;

comment on column public.pt_user_profiles.is_founder_study is
  'Plaza FOUNDER Study confirmada por admin.';
comment on column public.pt_user_profiles.is_founder_coach is
  'Plaza FOUNDER Coach confirmada por admin.';

-- Solicitud por plan: study | coach (también acepta pro | premium)
drop function if exists public.pt_request_founder_seat();

create function public.pt_request_founder_seat(p_plan text default 'study')
returns json
language plpgsql
security definer
...
begin
  ...
  tier := lower(trim(coalesce(p_plan, 'study')));
  if tier in ('pro', 'study') then
    tier := 'study';
    plan_label := 'Study';
  elsif tier in ('premium', 'coach') then
    tier := 'coach';
    plan_label := 'Coach';
  else
  ...
  already_founder := case
    when tier = 'study' then coalesce(prof.is_founder_study, false)
    else coalesce(prof.is_founder_coach, false)
  end;
  requested_at := case
    when tier = 'study' then prof.founder_study_requested_at
    else prof.founder_coach_requested_at
  end;
  ...
  subj := 'Solicitud de Founder ' || plan_label;
  ...
  if existing_id is not null then
    if tier = 'study' then
      update public.pt_user_profiles
      set founder_study_requested_at = coalesce(founder_study_requested_at, timezone('utc', now())),
          founder_requested_at = coalesce(founder_requested_at, timezone('utc', now()))
      where user_id = uid;
    else
      update public.pt_user_profiles
      set founder_coach_requested_at = coalesce(founder_coach_requested_at, timezone('utc', now())),
          founder_requested_at = coalesce(founder_requested_at, timezone('utc', now()))
      where user_id = uid;
    end if;
    ...
    return json_build_object(
      ...
      'is_founder_study', coalesce(prof.is_founder_study, false),
      'is_founder_coach', coalesce(prof.is_founder_coach, false),
      'founder_study_requested_at', case when tier = 'study' then coalesce(requested_at, timezone('utc', now())) else prof.founder_study_requested_at end,
      'founder_coach_requested_at', case when tier = 'coach' then coalesce(requested_at, timezone('utc', now())) else prof.founder_coach_requested_at end
    );
  end if;

  body := 'Quiero reservar una plaza FOUNDER ' || plan_label ||
    ' (próximamente, 40% de descuento, plazas limitadas por petición). ' ||
  ...
  values (new_thread_id, 'user', uid, body);

  if tier = 'study' then
    update public.pt_user_profiles
    set founder_study_requested_at = timezone('utc', now()),
        founder_requested_at = coalesce(founder_requested_at, timezone('utc', now()))
    where user_id = uid;
  else
    update public.pt_user_profiles
    set founder_coach_requested_at = timezone('utc', now()),
        founder_requested_at = coalesce(founder_requested_at, timezone('utc', now()))
    where user_id = uid;
  end if;
  ...
  return json_build_object(
    ...
    'is_founder_study', coalesce(prof.is_founder_study, false),
    'is_founder_coach', coalesce(prof.is_founder_coach, false),
    'founder_study_requested_at', case when tier = 'study' then timezone('utc', now()) else prof.founder_study_requested_at end,
    'founder_coach_requested_at', case when tier = 'coach' then timezone('utc', now()) else prof.founder_coach_requested_at end
  );
end;
$$;

revoke all on function public.pt_request_founder_seat(text) from public;
grant execute on function public.pt_request_founder_seat(text) to authenticated;

-- Admin update con Study/Coach
drop function if exists public.pt_admin_update_user(text, text, boolean, timestamptz, boolean);
...
create function public.pt_admin_update_user(
  ...
  p_is_founder_study boolean default null,
  p_is_founder_coach boolean default null
)
...
begin
  ...
  next_study := coalesce(p_is_founder_study, r.is_founder_study);
  next_coach := coalesce(p_is_founder_coach, r.is_founder_coach);
  -- Compat: p_is_founder solo afecta Study si no se envían flags nuevos
  if p_is_founder is not null and p_is_founder_study is null and p_is_founder_coach is null then
    next_study := p_is_founder;
  end if;
  ...
  set
    ...
    is_founder_study = next_study,
    is_founder_coach = next_coach,
    is_founder = (next_study or next_coach),
...
revoke all on function public.pt_admin_update_user(text, text, boolean, timestamptz, boolean, boolean, boolean) from public;
grant execute on function public.pt_admin_update_user(text, text, boolean, timestamptz, boolean, boolean, boolean) to authenticated;
